...
public class FlwTaskAssigneeServiceImpl implements IFlwTaskAssigneeService, HandlerSelectService {
    ...
    private List<UserDTO> getUsersByType(TaskAssigneeEnum type, List<String> ids) {
        // 1. 【String ID 分流】: 优先处理不需要转 Long 的类型
        if (type == TaskAssigneeEnum.SPEL) {
            return new ArrayList<>();
        }
        // 针对 USER 类型，直接使用 String 类型的 ids，避免非法数字转换异常
        if (type == TaskAssigneeEnum.USER) {
            // 注意：此处假设 userService.selectListByIds 已调整为接收 List<String>
            return userService.selectListByIds(ids);
        }
        // 2. 【Long ID 分流】: 仅针对确认为 Long 类型的业务场景进行转换
        // 只有代码走到这里，才能安全地认为 ids 里的内容都是数字
        List<Long> longIds = StreamUtils.toList(ids, Convert::toLong);
        return switch (type) {
            // USER 分支已在上文处理，此处移除，避免逻辑冗余
    ...
    private Map<String, String> getNamesByType(TaskAssigneeEnum type, List<String> ids) {
        // 1. 【String ID 组】: 直接处理不需要转换为 Long 的类型 (SPEL, USER)
        // 这样既避免了 unnecessary 的类型转换，也防止了非数字 UserID 导致的异常
        if (type == TaskAssigneeEnum.SPEL) {
            return spelService.selectRemarksBySpels(ids);
        }
        if (type == TaskAssigneeEnum.USER) {
            // 假设 userService 接收 String 列表并返回 Map<String, String>
            return userService.selectUserNamesByIds(ids);
        }

        // 2. 【Long ID 组】: 需要将 String ID 转换为 Long 进行查询的类型 (ROLE, DEPT, POST)
        // 在这里统一进行转换，确保只有真正的数字 ID 才会走入此逻辑
        List<Long> longIds = StreamUtils.toList(ids, Convert::toLong);

        Map<Long, String> longKeyMap = switch (type) {
            ...
            default -> Collections.emptyMap(); // 处理未知的 Long 类型枚举
        };

        // 3. 【结果统一化】: 将 Map<Long, String> 转换为 Map<String, String> 以统一返回类型
        if (MapUtil.isEmpty(longKeyMap)) {
            return Collections.emptyMap();
        }

        return longKeyMap.entrySet()
            .stream()
            .collect(Collectors.toMap(
                e -> Convert.toStr(e.getKey()), // Key: Long -> String
                Map.Entry::getValue             // Value: 保持不变
            ));
    }
