...
public class SysProfileController extends BaseController {
    ...
    public R<Void> updatePwd(@Validated @RequestBody SysUserPasswordBo bo) {
        ...
        if (!Objects.equals(bo.getOldPassword(), password)) {
            return R.fail("修改密码失败，旧密码错误");
        }
        if (Objects.equals(bo.getNewPassword(), password)) {
            return R.fail("新密码不能与旧密码相同");
        }
        //int rows = DataPermissionHelper.ignore(() -> userService.resetUserPwd(user.getUserId(), BCrypt.hashpw(bo.getNewPassword())));
        int rows = DataPermissionHelper.ignore(() -> userService.resetUserPwd(user.getUserId(), bo.getNewPassword()));
        if (rows > 0) {
            return R.ok();
        }
