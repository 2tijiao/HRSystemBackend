...
public class SysDeptController extends BaseController {
    ...
    @RepeatSubmit()
    @PostMapping("/archive")
    public R<Void> archive(@RequestBody SysDeptBo dept) {
        return deptService.archive(dept);
    }
    ...
    public R<Void> edit(@Validated @RequestBody SysDeptBo dept) {
        ...
        if (!deptService.checkDeptNameUnique(dept)) {
            ...
            } else if (deptService.checkDeptExistStaff(deptId)) {
                return R.fail("该部门下存在已分配且未冻结的员工，不能禁用!");
            }
        }
