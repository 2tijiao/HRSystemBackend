...
public interface SysDeptMapper extends BaseMapperPlus<SysDept, SysDeptVo> {
     ...
     * @return
     */
    default String selectNumById(Long deptId){
        SysDept dept = this.selectOne(
            new LambdaQueryWrapper<SysDept>()
                .eq(SysDept::getDeptId, deptId)
                .select(SysDept::getDeptNumber) // 仅查询deptNumber字段
        );
        return dept != null ? dept.getDeptNumber() : null;
    }

    default String getLevelById(Long deptId) {
        SysDept dept = this.selectOne(
            new LambdaQueryWrapper<SysDept>()
        ...
        return String.valueOf(ancestors.split(",").length);
